...
type ScreenProps = StackScreenProps<RootStackParamList, 'EventSearchPanel'>;

const EventSearchPanelScreen = ({ route, navigation }: ScreenProps) => {
    ...
    const { handleSubmit, control, formState, reset, watch, setValue } = useForm<FormValues>({
        defaultValues: {
            ...
            eventType: currentQuery.eventType ?? 'all',
    ...
    const onSearch = (data: FormValues) => {
        const { state, county, city, organization, eventType } = data;
            ...
            <KeyboardAwareScrollView>
                <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                    <View style={styles.contentContainerStyle}>
                        ...
                        <View style={[styles.itemContainterStyle, getStyle('pt-8'), styles.borderTopStyle]}>
                            ...
                            {user && myCircles.length > 0 && (
                                <TouchableOpacity
                                    style={[
                                        getStyle('mb-8'),
                                        query.onlyMyCicle ? styles.selectedItemStyle : styles.itemStyle,
                                    ]}
                                    onPress={() => {
                                        setQuery({
                                            ...query,
                                            onlyMyCicle: !query.onlyMyCicle,
                                        });
                                    }}
                                >
                                    <CTText
                                        fontFamily={fonts.montserrat.regular}
                                        color={query.onlyMyCicle ? colors.white : colors.primary}
                                        style={getStyle(['py-12', 'px-12'])}
                                    >
                                        Only My Circle
                                    </CTText>
                                </TouchableOpacity>
                            ...
                            {query.address && (
                                <View style={getStyle(['row', 'align-items-center', 'mb-8', 'mx-4'])}>
                                    <CTSelect
                                        name="state"
                                        style={getStyle('col')}
                                        placeholder="State"
                        ...
                        <View style={[styles.itemContainterStyle, getStyle(['pt-8', 'mx-4']), styles.borderTopStyle]}>
                            <CTSelect
                                name="eventType"
                                style={getStyle('col')}
                                placeholder="Tennis"
...
export default EventSearchPanelScreen;
