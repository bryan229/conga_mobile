...
const OtherCircleItem = ({ circle, onPress }: Props) => {
    const user = useAppSelector((state) => state.auth.user!);
    const theme = useTheme();
    const { colors } = theme;
    const styles = useMemo(() => createStyles(theme), [theme]);

    const requestedMe = circle.members.find(
            ...
            <View style={getStyle(['row', 'justify-between', 'align-items-center'])}>
                ...
                {requestedMe && (
                    <CTText color={colors.primary} fontFamily={fonts.montserrat.regular} size={9}>
                        Request at {moment(new Date(requestedMe.updatedDate)).format('MMM DD, YYYY')}
                    </CTText>
                )}
            </View>
        </TouchableOpacity>
...
export default OtherCircleItem;
