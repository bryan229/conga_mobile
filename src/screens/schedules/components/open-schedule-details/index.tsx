...
const OpenScheduleDetails = ({ schedule, canSelfCheckIn = false, callback }: Props) => {
    ...
    const [isSharing, setIsSharing] = useState<boolean>(() => {
        ...
        if ((schedule.members.length || 0) < reqPlayerCount) return true;
        return schedule.isSharing || false;
    });
    const [otherPlayers, setOtherPlayers] = useState<OtherPlayer[]>(schedule.members || []);
    const [loading, setLoading] = useState<boolean>(false);
    const [sharedReserveLoading, setSharedReserveLoading] = useState<boolean>(false);
        ...
        if (schedule.status === SCHEDULE_STATUS.CONFIRM) {
            if (venue.setting?.ruleSettings?.isRequireCheckIn) {
                if (schedule.isArrived)
                    return <Icon name="scan-circle" type={IconType.Ionicons} size={30} color={colors.primary} />;
    ...
    const reserveOpenTimeSlot = async () => {
        setLoading(true);
        try {
            let params: any = {
                _id: schedule._id,
    ...
    const reserveSharedTimeslot = async () => {
        setSharedReserveLoading(true);
        try {
            let params: any = { _id: schedule._id };
            await ScheduleApi.reserveShared(params);
            callback();
        } catch (error) {
            dispatch(handleError(error));
        }
        setSharedReserveLoading(false);
    };

    // this feature will be used only on circle message screen
    const cancelSharedTimeslot = async () => {
        setSharedReserveLoading(true);
        try {
            let params: any = {
                _id: schedule?._id,
            ...
            <View style={styles.otherPlayerContainer}>
                {(schedule.members || []).map((x) => (
                    <View style={styles.otherPlayerBadgeContainer} key={x.fullName}>
                        <Icon name="person-outline" size={18} type={IconType.Ionicons} color={colors.iconPrimary} />
    ...
    const isSharedSchedule = () => {
        return schedule.isSharing && ![SCHEDULE_STATUS.CANCELED, SCHEDULE_STATUS.OPEN].includes(schedule.status);
    };

    const getOwnerName = () => {
        let ownerName = schedule.owner.fullName;
        if (venue?.setting?.ruleSettings.isShowOtherPyLevel && schedule.owner.gameLevel)
    ...
    return (
        <View>
            <View>
                ...
                {!isSharedSchedule() ? (
                    <>
                        <CTSwitch
                            ...
                            value={isSharing}
                            disabled={reqPlayerCount > 0 && (schedule.members.length || 0) < reqPlayerCount}
                        ...
                        <View style={styles.otherPlayerContainers}>
                            {reqPlayerCount > 0 && (
                                <CTText color={colors.danger} size={9}>
                                    ...
                                    if other players is less than {reqPlayerCount}, it will default to sharing a
                                    timeslot.
                                </CTText>
                            )}
                            {otherPlayers.map((x) => (
                ...
                {isSharedSchedule() ? (
                    // actually, on open schedule page, if user alread reserved shared timeslot, that timeslot doesn't be displayed.
                    // so "cancel reservation" feature will be used only on circle message page
                    <CTButton
                        loading={sharedReserveLoading}
                        color={
                            (schedule.members ?? []).some((x) => x.email === user.email)
                                ? colors.secondary
                                : colors.primary
                        }
                        title={
                        ...
                        onPress={
                            (schedule.members ?? []).some((x) => x.email === user.email)
                                ? cancelSharedTimeslot
                                : reserveSharedTimeslot
                        }
                    />
                ) : (
                    <CTButton
...
export default OpenScheduleDetails;
